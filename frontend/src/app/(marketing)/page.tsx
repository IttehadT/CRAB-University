...
export default function Home() {
  ...
  return (
    <div className="bg-background font-sans text-foreground transition-colors duration-300 overflow-hidden">
      ...
      {/* ── COMBINED HERO SECTION: Text + 3D Carousel ── */}
      <section className="relative flex flex-col items-center justify-center px-4 pt-24 pb-16 text-center md:pt-32">
        <div className="absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-100/50 via-background to-background opacity-100 dark:from-blue-900/20"></div>
        
        {/* 1. The Classic Text Hero */}
        ...
        <div className="mt-10 mb-20 flex flex-wrap items-center justify-center gap-4">
          ...
          <Link href="#mission" className="rounded-xl border border-slate-200 bg-white px-8 py-4 font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800">
            Why I Built This
          </Link>
        </div>

        {/* 2. The Staggered 3D Carousel */}
      ...
      <section id="features" className="py-24 px-4 bg-slate-50 dark:bg-slate-950/50">
          ...
          >
            {sliderFeatures.map((feature, index) => (
              <Link 
                href={feature.href} 
                key={index}
                className="group relative min-w-[280px] snap-center overflow-hidden rounded-2xl border border-border bg-card p-8 shadow-sm transition-all duration-300 hover:-translate-y-2 hover:border-blue-500/50 hover:shadow-[0_10px_40px_-15px_rgba(37,99,235,0.15)] md:min-w-[320px]"
              >
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="mb-3 flex items-center gap-2 text-xl font-bold text-card-foreground group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                    {feature.label}
                  </h3>
                  
                  <div className="flex flex-wrap gap-2 mb-4">
                    {feature.isBeta && <span className="rounded bg-amber-500/10 px-2 py-1 text-[10px] font-bold tracking-wider text-amber-600 dark:text-amber-400">BETA</span>}
                    {feature.isNew && <span className="rounded bg-blue-500/10 px-2 py-1 text-[10px] font-bold tracking-wider text-blue-600 dark:text-blue-400">NEW</span>}
                    {feature.isAi && <span className="rounded bg-purple-500/10 px-2 py-1 text-[10px] font-bold tracking-wider text-purple-600 dark:text-purple-400 flex items-center gap-1">✨ AI</span>}
                  </div>
      ...
      <section id="mission" className="bg-slate-900 py-24 text-white dark:bg-slate-950">
        <div className="container mx-auto grid gap-12 px-4 md:grid-cols-2 md:items-center">
          <div>
            <h2 className="text-3xl font-bold leading-tight md:text-5xl">
              Why I Built <span className="text-blue-500">{siteConfig.brand?.name || "CRAB University"}</span>
            </h2>
          ...
          <div className="aspect-video rounded-2xl bg-slate-800 p-8 flex items-center justify-center border border-slate-700 shadow-2xl">
            <p className="font-mono text-slate-400">System Architecture Diagram</p>
          </div>
        </div>
      </section>
