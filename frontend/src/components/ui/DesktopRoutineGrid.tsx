...
export function DesktopRoutineGrid({ courses, showExams = true, forwardedRef, onRemoveCourse }: DesktopRoutineGridProps) {
  ...
  return (
    // 🔥 Changed to gap-4 with transparent background to create physical separation!
    <div ref={forwardedRef} className="min-w-[900px] flex flex-col w-full gap-4">
                          ...
                          {slotCourses.map((slot, i) => {
                            ...
                            return (
                              <div
                                  ...
                                  >
                                  {/* 🔥 CLOSE BUTTON (Now securely hidden during export) */}
                                  {onRemoveCourse && (
                                      <button
                                      onClick={(e) => {
                                          e.stopPropagation();
                                      ...
                                      className="export-hide absolute top-1.5 right-1.5 flex h-4 w-4 items-center justify-center rounded bg-destructive/15 text-destructive opacity-50 hover:bg-destructive hover:text-white hover:opacity-100 transition-all"
                                      title="Remove Course"
                                      >
                                      <svg
      ...
      {/* ── 2. EXAM SCHEDULE CARD (Now isolated with a gap) ── */}
      {showExams && examRows.length > 0 && (
        <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-border bg-muted/10">
            {/* 🔥 Bumped Font Size to 14px */}
            <p className="text-[14px] font-black text-foreground uppercase tracking-wider flex items-center gap-1.5">📋 Exam Schedule</p>
          </div>
      ...
      {/* ── 3. CRABU WATERMARK (Clean Text) ── */}
      <div className="w-full pt-1 pb-2 flex items-center justify-between px-2">
          <p className="text-[10.5px] font-black text-muted-foreground uppercase tracking-widest opacity-60">
            Academic Schedule
          </p>
          <p className="text-[10.5px] font-bold text-muted-foreground tracking-wide opacity-80">
            GENERATED BY <span className="text-primary font-black">CRAB University</span> • www.crabu.app
          </p>
      </div>
